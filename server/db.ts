...
export const dbService = {
  ...
  async upsertCustomerFromOrder(order: any) {
    ...
    const existingIdx = memoryDb.customers.findIndex(c => c.vendorId === vendorId && c.phone === customerPhone);
    if (existingIdx !== -1) {
  ...
  async getActiveSubscriptionPlans() {
    ...
    return memoryDb.subscriptionPlans.filter(plan => plan.isActive);
  },
  ...
  async getSubscriptionPlanById(id: string) {
    ...
    return memoryDb.subscriptionPlans.find(plan => plan.id === id) || null;
  },
  ...
  async createSubscriptionPlan(planData: any) {
    ...
    memoryDb.subscriptionPlans.push(planData);
    return planData;
  },
